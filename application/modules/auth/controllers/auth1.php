...
class Auth extends CI_Controller {
  ...
  function index(){
	...
	{
		$fp=fopen('/var/www/stlite/assets/files/codenameblock.json','r');
		$contents = fread($fp,filesize('/var/www/stlite/assets/files/codenameblock.json'));
		$pos = strpos($contents, $username);
		return $pos;		
	...
	function login()
	{
		...
		$allowed = array('saumyajitdey69','tj2807','mj','sachin_patil','sushrut111');
		$data['title'] = "Login";
		...
		if ($this->form_validation->run() == true)
		{
			...
			$pos=$this->check_user_login($username);	
				
			if($pos == false && !in_array($username, $allowed))
			{
				$this->session->set_flashdata('danger','You are currently not allowed for OMAHA. Only MTECH 2nd Year boys will be allowed. Please message us immediately at our <a href="www.facebook.com/wsdc.nitw">Facebook Page</a> if you think you should be allowed');
				redirect('auth/login', 'refresh');
			}
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				//if the login is suc essful
				//redirect them back to the home page
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				//---------------------Single time Single login implementation----------------
				//Author : Tejas Niphadkar
				//Get the current session id here
				//Get current userid
				//kill all the existing sessions with that userid
				//insert userid in current session row using sessid
				//----------------------------------------------------------------------------
				$this->load->model('ion_auth_model');
				$userid = $this->session->userdata('user_id');
				$session_id = $this->session->userdata('session_id');
				//echo $userid;
				$this->ion_auth_model->kill_sessions($userid);
				$this->ion_auth_model->update_userid($session_id,$userid);
				redirect('hostels', 'refresh');
			}
			else
			{
	...
	public function create_general_user()
	{
		...
		$this->form_validation->set_rules('user_id', $this->lang->line('create_user_validation_userid_label'), 'xss_clean|trim|ctype_alnum|required|is_unique['.$tables['users'].'.username]');
		...
		$this->form_validation->set_rules('rollno', $this->lang->line('create_user_validation_rollno_label'), 'xss_clean|trim|required|min_length[6]|max_length[7]|ctype_digit|is_unique['.$tables['student_data'].'.roll_number]');
	...
	function _render_wsdc_page($view, $data=null, $render=false)
	{
		...
		$view_html = array( $this->load->view('base/header', $data, $render),
			$this->load->view('hostels/menu/header', $data, FALSE),
			$this->load->view($view, $this->viewdata, $render),
			$this->load->view('hostels/menu/footer', $data, FALSE),
